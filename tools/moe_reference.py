...
- deepseek.npz: the same for the router of a `DeepseekV3MoE`, with a nonzero
  `e_score_correction_bias` so the selection bias is exercised, and without
  the block output, because that block adds a shared expert Dew's `SparseMLP`
  does not have.
...
def main() -> None:
    ...
    (out / "config.json").write_text(
        json.dumps({"mixtral": MIXTRAL, "deepseek": DEEPSEEK}, indent=2) + "\n")
    write_mixtral(out)
    write_deepseek(out)
