...
def optim_config(width: int, steps: int, batch: int, seq_len: int) -> OptimConfig:
    ...
    return OptimConfig(
        ...
        learning_rate_schedule="power", learning_rate_peak=0.01,
        learning_rate_warmup_steps=max(round(5_000 * scale), 1),
        power_a=4.0 * batch, power_b=-0.51, power_c=float(batch * seq_len),
        learning_rate_decay_start=round(DECAY_START * scale), learning_rate_end=0.0,
