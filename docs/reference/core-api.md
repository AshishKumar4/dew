...
`SourceSchedule.from_config` reads one published `scheduler_config.json` into the native process, solver and time grid that reproduce it. Seventeen classes load: DDIM, PNDM, DDPM, LMS, Euler, Euler ancestral, Heun, KDPM2 and KDPM2 ancestral, DPM-Solver multistep, singlestep and SDE, DEIS, UniPC, the EDM DPM-Solver, LCM and TCD. Each is read through the controls that class actually declares, with that class's own defaults, so a key another class declares does not change this one; the reconstruction keeps its evaluation grid (linspace, leading and trailing spacing with `steps_offset`, the distilled LCM/TCD grid out of `original_inference_steps`), its sigma grid (interpolated, Karras, exponential or beta, with the rounding and the integer truncation each class applies to the recovered model times), finite `lambda_min_clipped`, zero-terminal-SNR beta rescaling with the near-zero terminal alpha the classes that patch it substitute, both terminal sigmas, the fixed DDIM/PNDM transfer stride against DDPM's grid-interval policy, DDPM's two fixed posterior variances, `clip_sample` and dynamic thresholding in the conversion order the class applies them (including where epsilon is re-derived from the limited x_0, and before LCM's consistency boundary), and EDM's own sigma_min/sigma_max/sigma_data/rho convention with `c_noise = log(sigma) / 4` and a signed `c_out`. A class that evaluates the model twice per interval carries the source's interpolated stage row in its grid, so the solver's own second evaluation reads the source's sigma and model time there while the walk still visits one point per interval. An EDM checkpoint declares no beta table, so its `training_process()` is EDM's log-normal sigma draw over the same preconditioning rather than a VP law. A control a class declares and this reconstruction does not read fails at load rather than being ignored: `use_lu_lambdas`, `use_flow_sigmas`, UniPC's `solver_p`, non-linear sigma interpolation, continuous model times, learned or log-space wide variances, published PNDM velocity-domain history, and the limits the published TCD step and epsilon-prediction UniPC step do not read. Native PNDM itself integrates epsilon history. `tools/diffusers_source_reference.py` records each class's default and a discriminating nondefault file, walked with the actual scheduler objects: a stochastic class is fed the draws the native walk folds per step, `DPMSolverSDEScheduler` runs its own `torchsde` tree and the native walk integrates the draws it answered, and each case also records the reference's own float32 gradient, which is how close a float32 reconstruction can come. A grid whose recovered model times repeat their first value is refused at `sampling()`: the source looks its starting step up by that value, finds the second and runs off the end of its sigma table, which the tool records as the failure it raises.
