...
Correction, 2026-09-22: the source links in this note point at line ranges from `d0d04c2`, and some have moved. `src/dew/training/objective_trainer.py` does not exist any more; the step, EMA and `grad_accum_steps` code the fairness table links to is now in `src/dew/training/trainer.py`. Fixes 4 and 5 below are done: `src/dew/telemetry/instrumentation.py:1-10` reads the step FLOPs off the optimized HLO instead of `cost_analysis()`, and `tools/benchmark_step.py` defaults to 100 steps (`tools/benchmark_step.py:460`) and reports p10/p50/p90 (`tools/benchmark_step.py:817`). Fix 1 is not the default for the causal transformer: its `attention_impl` field defaults to `None` (`src/dew/nn/backbones/causal_transformer.py:1420`), which runs the reference path (`src/dew/nn/attention.py:40`). For fix 3, the chunked head computes token accuracy from a running argmax; see [lm-head.md](lm-head.md).
