...
| Model definitions as parity references | depend | transformers 5.16.1 in the pinned venv, per family lines in `docs/research/model-families.md:46-73`; MaxText's own `models/` for the sharding shapes (27 files, `docs/research/google-jax-stack.md:142`) | Apache 2.0 | Dew loads the real checkpoint through `dew.interop.hf_decoders` and compares logits. MaxText's model files are read for the axis annotations, not copied | Per family, the parity tests already specified in `docs/research/model-families.md:1146-1245` |
