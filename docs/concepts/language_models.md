...
The vocabulary loss runs in float32: the head multiplies fp32 states by the table as stored. `bf16_head=True` on the backbone multiplies both as bf16 and sums in float32, backward included; it is 1.6x faster on an L4 and an RTX 4080 and changes the loss by less than the bf16 head's own rerun spread ([performance](../performance.md#kernel-choices-per-generation-2026-09-22)). The step reports `ce`, `perplexity` and `token_accuracy`. `token_accuracy=False` on `LMObjective`, `--no-token-accuracy` on the recipe, drops the accuracy and the pass over every logit it costs: 0.77 ms of the head's 8.0 ms on a TPU v6e. `head_chunks` sets how many vocabulary slices the loss scores; the default is four. Chunking can lower peak memory, but it adds work, and the backend compiler may rewrite it. How much memory it saves depends on the vocabulary size, sequence length, batch size and the compiled executable. There is no fixed saving.
...
A config field Dew cannot express is refused by name. The exceptions are the fields in `_INERT_FIELDS` (`dew/interop/hf_decoders.py`), which transformers 5.16.1 neither declares nor reads, such as SmolLM2's `transformers.js_config`, Qwen2.5's `use_mrope: false` and the Mamba2 ports' `rms_norm`. A source whose weights Dew cannot read says why. A PyTorch-pickle repo names SFconvertbot's safetensors pull request (`revision="refs/pr/N"`) when the Hub has one for that commit. GGUF files and MLX quantization are named as such.
