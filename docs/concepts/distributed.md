...
There is no 1F1B schedule beside GPipe, by design. A 1F1B step frees each microbatch's activations as soon as its backward runs, but autodiff generates the backward from the forward program and cannot interleave what is not written there, so inside one compiled step the choice is between recomputing every forward inside its backward (a third more compute, and the recomputed forwards sow the routers' collections a second time, which the balance loss cannot read) and driving per-stage forward and backward programs from Python. The second form was built as a spike: two stages over four microbatches with explicit per-stage VJPs matches the plain loop's gradients to 9.3e-09, but the step becomes a Python loop of per-stage dispatches, sixteen here against GPipe's one compiled step, with one executable per stage program. GPipe holds every microbatch's activations to the end of the forward sweep instead, and at depth 24 on the simulated mesh that step compiles once in 6.87 s and runs in 84.73 ms (`python tools/benchmark_stack.py --depths 24 --scan False --stage 2 --microbatches 4 --fsdp 4`). A 1F1B schedule lands if the memory ever forces it, as per-stage programs, not as a `MeshSpec` flag.
