...
`dew.objectives.rl.EpisodeRollout` collects multi-turn episodes through a user-supplied environment and a model-bound inference callable. It uses the ordinary `Trainer(rollout=...)` capability and `GRPOObjective`; it supplies no executor or sandbox. The native inference callable is `functools.partial(dew.sampling.generate, model)`. Each call receives the collection's variables, a batch of exact context token IDs, a response budget, a key and `Sampling`, and returns `Generation` with raw and behavior log-probabilities.
