...
`dew.objectives.diffusion.DiffusionObjective` is the default: sample a noise level from the schedule, corrupt the sample, predict, transform the prediction, weight the per-sample losses by the schedule's weights. Its validation step is a sampler run, and its artifacts are generated images or videos. If an autoencoder is configured it encodes the batch first, which is how latent diffusion runs on the same path. The model then works on latents. Its channel fields, `output_channels` or `in_channels`/`out_channels` on `diffusers_unet_simple`, are the autoencoder's `latent_channels`, 4 for the Stable Diffusion VAE. `DiffusionInputConfig` still takes the pixel shape, and the trainer derives the latent grid from it and the downscale factor.
