...
- `DataConfig(dataset, dataset_path, dataset_seed, batch_size, image_size, val_steps_per_epoch, loader, augmentation_mode, worker_count, read_thread_count, read_buffer_size, worker_buffer_size)`. `loader` is `auto`, `grain` or `online`.
...
Each recipe subclasses `RunConfig` with the knobs its objective needs. `DiffusionRunConfig` adds `noise_schedule`, `min_snr_gamma`, `flow_shift`, `autoencoder`, `autoencoder_opts`, `val_metrics` and `dataset_test`. `JepaRunConfig` adds `predictor`, `frames_per_sample`, `num_target_blocks`, `target_scale`, `target_aspect`, `momentum`, `momentum_steps`, `probe_classes`, `probe_label_key` and `knn_k`.
