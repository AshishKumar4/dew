...
Run records in `dew.telemetry.records` are `RunRecord` (resolved model/data/optimizer configuration and package versions), `FitStarted`, `CheckpointRequested`, `ProfileWindow`, `FitEnded`, and `TrialFinished` (one sweep trial). Checkpoint requests record asynchronous submission, not durability; existing checkpoint waits are unchanged. The `ProfileWindow` record names the directory a `Trainer` profile window wrote and the number of steps it traced, and is reported once that trace has stopped, without per-step layer tensor copies. A standalone `dew.profile` capture writes its own directory and reports no record. Data contents and source revisions are not automatically hashed: include their identities in the run summary when needed.
