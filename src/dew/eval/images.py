...
def _clip_image_text_cosine(model, processor, artifact, batch, field):
    """Per-image cosine on complete, equally sized image and prompt batches."""
    ...
    if (np.shape(text["input_ids"])[0] != count
            or np.shape(text["attention_mask"])[0] != count):
        raise ValueError(
            f"the artifact holds {count} rows and batch[{field!r}] "
            f"{np.shape(text['input_ids'])[0]}; CLIP requires equal prompt counts")
...
def clip(modelname: str = "openai/clip-vit-large-patch14", field: str = "text") -> ImageMetric:
    ...
    def measure(artifact, batch):
        ...
        return 1.0 - _clip_image_text_cosine(model, processor, artifact, batch, field)
...
def clip_score(modelname: str = "openai/clip-vit-large-patch14",
    ...
    def measure(artifact, batch):
        ...
        return 100.0 * jnp.maximum(cos, 0.0)
