...
def build_mesh(spec: MeshSpec = MeshSpec(), devices: Optional[list] = None) -> Mesh:
    ...
    where long sequences split. Sizes of 1 degenerate to plain data
    parallelism, so the same code path serves every topology without a flag.
    Axes are Auto so GSPMD infers the collectives.
    """
