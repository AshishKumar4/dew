...
`source.py` reads one scheduler file into a policy; these are the schedules
that policy hands to `Process`. Each is an ordinary Dew schedule, so a solver
reads its rates, its model time and its prior the way it reads any other:
the training beta table indexed by t, a paired sigma/model-time table in
variance-exploding or normalized-VP coordinates, the same table refined with
the stage row a two-evaluation solver reads, and a rectified-flow table whose
signal and noise sum to one.
"""
...
class FlowGrid(_PairedGrid, NoiseScheduler):
    ...
    signal and noise sum to one rather than their squares. Model times are the
    sigmas times the training count, which is where the source's `timesteps`
    come from, and the prior at sigma 1 is the unit Gaussian.
    """
