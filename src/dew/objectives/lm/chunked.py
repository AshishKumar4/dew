...
comparison keeps the lowest index among equals, as `jnp.argmax` does for
the whole row. The tile matmuls accumulate in float32 and the
reductions run in float32, so the numbers are the ones the full pass produced.

The tiles are kept for the backward pass. On one RTX 4080 at this shape,
keeping them costs 0.32 GiB and runs the head forward and backward in 49.71
ms, while `jax.checkpoint` around the tile saves the 0.32 GiB and costs
67.19 ms. Recomputing every tile is a fourth pass of the head matmul.
Storing four tiles is 1.08 GiB below the full-vocabulary path, which holds
the logits and their softmax gradient at once. MaxText's vocabulary tiling
recomputes, behind a hand-written `custom_vjp` over a `lax.scan`, because its
tiles carry sharding constraints across a much larger vocabulary and many
devices
(https://github.com/AI-Hypercomputer/maxtext, maxtext/utils/vocabulary_tiling.py).
The numbers above are what a single 16 GiB card says; the reproduction
...
def chunked_cross_entropy(hidden, head_weight, targets, chunks: int, *,
    ...
    # fp32 states, the dtype the full-vocabulary head computes in.
    ...
    for start, stop in bounds:
        ...
        # equal logits the lowest column wins, which is jnp.argmax's rule on
        # the row.
