...
def test_the_batch_sequence_dimension_takes_the_sequence_axis():
    ...
    batch = shard_batch(mesh, {"text": np.zeros((BATCH, SEQ_LEN + 1), np.float32)})["text"]
...
def test_a_width_the_sequence_axis_cannot_split_stays_replicated():
    ...
    mesh = build_mesh(MeshSpec(fsdp=4, sequence=2))
    batch = shard_batch(mesh, {"text": np.zeros((BATCH, SEQ_LEN + 2), np.float32)})["text"]
...
TOPOLOGIES = {
    ...
    # and the stage axis beside them: the two-layer model splits into two
    # stages of one layer, fed four microbatches of two rows.
...
def test_topologies_agree_with_data_parallel():
    ...
    # Observed 7.2e-07 at most against the fsdp run over all 30 steps on
    # CPU, the pipelined topologies included; the tolerance is 1e-6 because
    # a different collective order on another backend is allowed to round
    # differently.
