...
of bf16's - unless the dot carries an algorithm, which names the arithmetic
instead of the storage: `algorithm=dot_bf16_bf16_f32` rounds both operands to
bf16, multiplies them there and accumulates in fp32, whatever dtype they are
stored as. A dot with that algorithm is a bf16 dot however its operands read,
so this file asks the algorithm first and the operand dtypes second. The
backward pass of an fp32-result head is where the difference lives: its
cotangent is fp32 and nothing can make it otherwise, so the algorithm is how
that product is asked to run at bf16 (jax's own xla attention sets the same
preset for the same reason, jax-ml/jax#24047). That is findings 1 and 3 of the
MFU audit
...
On an accelerator the fused attention kernel is one custom call rather than
two dots, and it takes its fp32 log partition beside the bf16 matrices it
multiplies; what it multiplies is the operands shaped like its result, which
is what `Matmul.multiplied` reads.
...
# XLA's dot algorithm, `dot_<lhs>_<rhs>_<accumulate>`: the types the machine
...
class Matmul:
    ...
    algorithm: str
    """XLA's dot algorithm, empty where the instruction names none. Where it
    is named it decides the arithmetic: `dot_bf16_bf16_f32` multiplies bf16
    and accumulates fp32 even when an operand is stored fp32."""
    flops: float
    ...
    @property
    def multiplied(self) -> tuple[str, ...]:
        """The types the machine multiplies.

        An algorithm names them outright. A fused attention call carries its
        softmax statistics beside the matrices it multiplies - the log
        partition, one axis shorter than the result, fp32 on purpose and an
        operand of no product - so a call reads the operands shaped like its
        result. Everything else multiplies what it is handed.
        """
        if self.algorithm:
            return tuple(self.algorithm.removeprefix("dot_").split("_")[:2])
        operands = self.operands
        if self.op == "custom-call":
            operands = tuple(one for one in operands if _rank(one) == _rank(self.result))
        return tuple(operand.split("[")[0] for operand in operands)

    @property
    def fp32(self) -> bool:
        return any(kind in FP32 for kind in self.multiplied)

    def __str__(self) -> str:
        return (f"{self.flops:>15,.0f} FLOPs  %{self.name} = {self.result} "
                f"{self.op}({', '.join(self.operands)})"
                + (f" algorithm={self.algorithm}" if self.algorithm else "")
...
def _rank(typed: str) -> int:
    """The number of axes `f32[1,2,12]` spells, 0 for a scalar."""
    axes = typed.partition("[")[2].rstrip("]")
    return len(axes.split(",")) if axes else 0
...
def matmuls(text: str) -> list[Matmul]:
    ...
    for line in text.splitlines():
        ...
        algorithm = _ALGORITHM.search(attributes)
        found.append(Matmul(
            ...
            algorithm.group(1) if algorithm else "",
...
def test_the_lm_head_multiplies_bf16_states(rng):
    ...
    of a dense step and 7.96% of the MoE step on the RTX 4080 when both of
    its operands were fp32 (audit finding 7). Under bf16 compute the chunked
    loss multiplies bf16 by bf16 and accumulates in fp32, forward and
    backward, which is all an exact logsumexp needs."""
