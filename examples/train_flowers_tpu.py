...
def smoke_config(config: Config, out: Path) -> DiffusionRunConfig:
    ...
    return DiffusionRunConfig(
        ...
        # `evaluate` does no iterator work for a pass with no consumers, so a
        # smoke that wants the validation path walked names a metric that
        # downloads nothing; fid and clip_score both pull their own weights.
        val_metrics=("psnr",),
